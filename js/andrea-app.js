...
const SUPABASE_URL = 'https://hhdcobkizdzpqbcdpnea.supabase.co';
...
class AndreaApp {
    ...
    async sendMessageNormal(text) {
        ...
        // Resetear la altura del textarea a su estado original
        this.ui.resetTextareaHeight(input);
    ...
    showWriterModePrompt(originalText) {
        ...
        modal.innerHTML = `
            <div class="writer-prompt-content">
                <p>Do you want Andrea to enter writing mode to generate the formatted text?</p>
                <div class="writer-prompt-buttons">
                    <button id="writer-prompt-yes">Yes, enter write mode</button>
                    <button id="writer-prompt-no">No, answer normally</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
    ...
    async generateWriterText() {
        ...
        outputDiv.innerHTML = '<p class="loading">Generating...</p>';
...
class UIHelper {
    ...
    resetTextareaHeight(textarea) {
        textarea.style.height = 'auto';
        textarea.style.height = ''; // Restablecer a valor por defecto
    }
